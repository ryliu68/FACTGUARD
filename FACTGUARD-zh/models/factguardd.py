...
from .factguard import FACTGUARDModel

class FACTGUARDDModel(torch.nn.Module):
    """
    FACTGUARDD model for knowledge distillation.
    Uses a teacher FACTGUARDModel and a student model with transformer and attention layers.
    """
    def __init__(self, config):
        super(FACTGUARDDModel, self).__init__()

        # Teacher model (pretrained FACTGUARD)
        self.teacher = FACTGUARDModel(config)

        # Student model backbone (Bert)
        self.bert = BertModel.from_pretrained(config['bert_path'],ignore_mismatched_sizes=True).requires_grad_(False)
        ...
        # Student model transformer block, attention, and MLP head
    ...
    def forward(self, **kwargs):
        """
        Forward pass for FACTGUARDD model.
        Returns both student and teacher features for distillation.
        """
...
class Trainer():
    """
    Trainer class for FACTGUARDD model, including training, validation, and testing.
    ...
    def train(self, logger = None):
        """
        Training loop for FACTGUARDD model with distillation loss.
        """
        st_tm = time.time()
        writer = self.writer
        ...
        self.model = FACTGUARDDModel(self.config)
        self.model.teacher.load_state_dict(torch.load(self.config['teacher_path']))
        if self.config['use_cuda']:
            self.model = self.model.cuda()

        loss_crit = nn.MSELoss()  # Distillation loss
        loss_fn = torch.nn.BCELoss()  # Classification loss

        optimizer = torch.optim.AdamW(params = self.model.params, lr = self.config['lr'], weight_decay = self.config['weight_decay'])
        ...
        for epoch in range(self.config['epoch']):
            ...
            for step_n, batch in enumerate(train_data_iter):
                ...
                # Distillation loss between student and teacher features
                loss_final = loss_crit(res['s_final_feature'], res['t_final_feature'])
            
                # Total loss: classification + distillation
    ...
    def test(self, dataloader):
        """
        Validation/testing loop for FACTGUARDD model.
        Computes both classification and distillation loss.
        """
    ...
    def predict(self, dataloader):
        """
        Prediction loop for FACTGUARDD model.
        """
