...
class FACTGUARDModel(torch.nn.Module):
    """
    FACTGUARD model without LLM usefulness loss. Uses Bert backbone and attention/MLP heads.
    """
    ...
    def forward(self, **kwargs):
        ...
        mutual_content_FTR_2, _ = self.cross_attention_content_2(content_feature_2, FTR_3_feature, content_masks)
        expert_2 = torch.mean(mutual_content_FTR_2, dim=1)
    
        mutual_content_FTR_3, _ = self.cross_attention_content_3(content_feature_2, FTR_3_feature, content_masks)
        expert_3 = torch.mean(mutual_content_FTR_3, dim=1)
    
        mutual_FTR_content_2, _ = self.cross_attention_ftr_2(FTR_3_feature, content_feature_2, FTR_2_masks)
    ...
    def train(self, logger = None):
        ...
        for epoch in range(self.config['epoch']):
            ...
            for step_n, batch in enumerate(train_data_iter):
                ...
                loss_hard_aux = loss_hard_aux_fn(res['hard_ftr_2_pred'], hard_ftr_2_label.float()) + \
                                loss_hard_aux_fn(res['hard_ftr_3_pred'], hard_ftr_3_label.float())
            
                loss_simple_aux_fn = torch.nn.CrossEntropyLoss()
                loss_simple_aux = loss_simple_aux_fn(res['simple_ftr_2_pred'], simple_ftr_2_label.long()) + \
