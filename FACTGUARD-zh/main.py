...
# Prepare configuration dictionary for the experiment
config = {
        'use_cuda': True,
        'seed': args.seed,
...
if __name__ == '__main__':
    ...
    print('rationale_usefulness_evaluator_weight=', config['model']['rationale_usefulness_evaluator_weight'],
          'llm_text_encoder_weight=', config['model']['llm_text_encoder_weight'])
    print('before in config')
    print(config)
    # Run training and get best metrics
    best_metric = Run(config=config, writer=writer).main()

    # Save best metrics to a JSON log file
    save_dir = './logs/log'
