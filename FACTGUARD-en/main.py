...
parser.add_argument('--model_name', type=str, default='FACTGUARD')
...
parser.add_argument('--recurrence', type=float, default = 0.0)
parser.add_argument('--expert_interaction_method', type=str, default = 'cross_attention')
parser.add_argument('--llm_text_encoder_weight', type=float, default = -1)
...
# Build config dictionary for training
config = {
        'use_cuda': True,
        'seed': args.seed,
        ...
        'model':
            ...
            'llm_text_encoder_weight': args.llm_text_encoder_weight,
...
if __name__ == '__main__':
    ...
    print('rationale_usefulness_evaluator_weight=',config['model']['rationale_usefulness_evaluator_weight'] ,'llm_text_encoder_weight=',config['model']['llm_text_encoder_weight'] )
    print('before in config')
    print(config)
    # Run training and grid search
    best_metric = Run(config = config, writer = writer).main()

    # Save best metric to log file
    save_dir = './logs/log'
