...
class FACTGUARDDModel(torch.nn.Module):
    """
    Distillation version of FACTGUARD model, using teacher-student structure.
    """
    def __init__(self, config):
        super(FACTGUARDDModel, self).__init__()

        # Teacher model (FACTGUARD)
        self.teacher = FACTGUARDModel(config)

        # Student backbone (Roberta)
        self.bert = RobertaModel.from_pretrained(config['bert_path']).requires_grad_(False)
        ...
        # Student transformer block, attention, and MLP head
    ...
    def forward(self, **kwargs):
        ...
        # Output student and teacher features for distillation loss
        res = {
...
class Trainer():
    """
    Trainer class for FACTGUARDDModel, including training, validation, and testing.
    """
    def __init__(self,
                 config,
                 writer
    ...
    def train(self, logger = None):
        """
        Training loop for distillation model.
        """
        st_tm = time.time()
        writer = self.writer
        ...
        for epoch in range(self.config['epoch']):
            ...
            for step_n, batch in enumerate(train_data_iter):
                ...
                # Distillation loss (student vs teacher features)
    ...
    def test(self, dataloader):
        """
        Validation/testing loop for distillation model.
        """
        loss_fn = torch.nn.BCELoss()
    ...
    def predict(self, dataloader):
        """
        Prediction loop for distillation model.
        """
