...
'''The attn_content  represents f_N, reweight_expert_2 represents the one with a weight tending to 0, and reweight_expert_3 represents the one with a weight tending to y_llm.'''
class FACTGUARDModel(torch.nn.Module):
    """
    Main model for FACTGUARD, including backbone, attention, and MLP heads.
    """
    def __init__(self, config):
        ...
        # Optionally use Bert backbone (commented out)
        # self.bert_content = BertModel.from_pretrained("google-bert/bert-base-uncased",ignore_mismatched_sizes=True).requires_grad_(False)
        # self.bert_FTR = BertModel.from_pretrained("google-bert/bert-base-uncased",ignore_mismatched_sizes=True).requires_grad_(False)
    ...
    def forward(self, **kwargs):
        ...
        mutual_FTR_content_2, _ = self.cross_attention_ftr_2( \
            FTR_3_feature, content_feature_2, FTR_2_masks)
        ...
        # Content attention
        attn_content1, _ = self.content_attention(content_feature_1, mask=content_masks)
        attn_content2, _ = self.content_attention(content_feature_1, mask=content_masks)
    ...
    def train(self, logger = None):
        ...
        for epoch in range(self.config['epoch']):
            ...
            for step_n, batch in enumerate(train_data_iter):
                ...
                # Auxiliary losses for rationale heads，simple_ftr_3_label=0，hard_ftr_3_label=y_llm
    ...
    def predict(self, dataloader):
        ...
        for step_n, batch in enumerate(data_iter):
            with torch.no_grad():
                ...
                batch_label = batch_data['label']
                batch_llm_label = batch_data['FTR_3_pred']
                batch_input_data = {**self.config, **batch_data}
                res = self.model(**batch_input_data)
                ...
                id.extend(batch_id.detach().cpu().numpy().tolist())
                llm_label.extend(batch_llm_label.detach().cpu().numpy().tolist())
