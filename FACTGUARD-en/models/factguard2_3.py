...
class FACTGUARDModel(torch.nn.Module):
    """
    FACTGUARD model for multi-class classification, using Roberta backbone and attention/MLP heads.
    """
    def __init__(self, config):
        super(FACTGUARDModel, self).__init__()

        # Load pretrained Roberta models for content and FTR features
        self.bert_content = RobertaModel.from_pretrained(config['bert_path'], ignore_mismatched_sizes=True).requires_grad_(False)
        self.bert_FTR = RobertaModel.from_pretrained(config['bert_path'], ignore_mismatched_sizes=True).requires_grad_(False)
        ...
        # Hard rationale heads for FTR_2 and FTR_3
    ...
    def forward(self, **kwargs):
        ...
        # Extract features for FTR_2 and use as content features
        FTR_2_feature = self.bert_content(FTR_2, attention_mask = FTR_2_masks)[0]
        content_feature = FTR_2_feature
        attn_content1, _ = self.content_attention(content_feature, mask=FTR_2_masks)
        attn_content2, _ = self.content_attention(content_feature, mask=FTR_2_masks)
        attn_content = (attn_content1 + attn_content2) / 2
      
        # Extract features for FTR_3
        FTR_3_feature = self.bert_FTR(FTR_3, attention_mask = FTR_3_masks)[0]
        content_feature_2 = FTR_2_feature

        # Cross attention and expert feature extraction
        mutual_content_FTR_2, _ = self.cross_attention_content_2(content_feature_2, FTR_3_feature, FTR_2_masks)
        expert_2 = torch.mean(mutual_content_FTR_2, dim=1)
    
        mutual_content_FTR_3, _ = self.cross_attention_content_3(content_feature_2, FTR_3_feature, FTR_2_masks)
        expert_3 = torch.mean(mutual_content_FTR_3, dim=1)

        mutual_FTR_content_2, _ = self.cross_attention_ftr_2(FTR_3_feature, content_feature_2, FTR_3_masks)
        mutual_FTR_content_2 = torch.mean(mutual_FTR_content_2, dim=1)

        mutual_FTR_content_3, _ = self.cross_attention_ftr_3(FTR_3_feature, content_feature_2, FTR_3_masks)
        mutual_FTR_content_3 = torch.mean(mutual_FTR_content_3, dim=1)

        # Hard rationale predictions
        ...
        # Concatenate all features for final aggregation
        ...
        gate_value = reweight_score_ftr_3
    ...
    def train(self, logger = None):
        ...
        for epoch in range(self.config['epoch']):
            ...
            for step_n, batch in enumerate(train_data_iter):
                ...
                # Get rationale labels for hard and simple heads
                ...
                # Auxiliary losses for rationale heads
                loss_hard_aux_fn = torch.nn.BCELoss()
                loss_hard_aux =  loss_hard_aux_fn(res['hard_ftr_2_pred'], simple_ftr_3_label.float()) + loss_hard_aux_fn(res['hard_ftr_3_pred'], hard_ftr_3_label.float())
            
                loss_simple_aux_fn = torch.nn.CrossEntropyLoss()
                loss_simple_aux =  loss_simple_aux_fn(res['simple_ftr_2_pred'], label.long()) + loss_simple_aux_fn(res['simple_ftr_3_pred'], simple_ftr_3_label.long())

                # Total loss
                loss = loss_classify
                loss += self.config['model']['rationale_usefulness_evaluator_weight'] * loss_hard_aux / self.num_expert
                loss += self.config['model']['llm_text_encoder_weight'] * loss_simple_aux / self.num_expert
