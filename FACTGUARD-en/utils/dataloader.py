...
# Label mapping for multi-class rationale prediction
label_dict_ftr_pred = {
    "real": 0,
    "fake": 1,
    "other": 2,
...
def word2input(texts, max_len, tokenizer):
    """
    Convert a list of texts to token ids and attention masks using the tokenizer.
    Args:
...
def get_dataloader(path, max_len, batch_size, shuffle, bert_path, data_type, language):
    """
    Build a PyTorch DataLoader for the specified dataset.
    Args:
        path: Path to the data file (json).
        max_len: Maximum sequence length for tokenization.
        batch_size: Batch size for DataLoader.
        shuffle: Whether to shuffle the data.
        bert_path: Path or name of the pretrained model.
        data_type: Type of data ('rationale' supported).
        language: Language of the data (unused).
    Returns:
    ...
    if data_type == 'rationale':
        ...
        # Tokenize content and rationale fields
        ...
        # Build TensorDataset for DataLoader
